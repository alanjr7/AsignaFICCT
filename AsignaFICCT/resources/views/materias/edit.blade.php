...
<div class="max-w-2xl mx-auto">
    <div class="bg-white shadow rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Editar Materia: {{ $materia->sigla_materia }}</h2>
        </div>
        
        <form action="{{ route('materias.update', $materia) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="sigla_materia" id="sigla_materia" value="{{ old('sigla_materia', $materia->sigla_materia) }}" 
                ...
                <div>
                    ...
                    <input type="text" name="nombre_materia" id="nombre_materia" value="{{ old('nombre_materia', $materia->nombre_materia) }}"
                ...
                <div>
                    ...
                    <select name="nivel" id="nivel" 
                            ...
                            required>
                        @for($i = 1; $i <= 10; $i++)
                            <option value="{{ $i }}" {{ old('nivel', $materia->nivel) == $i ? 'selected' : '' }}>Nivel {{ $i }}</option>
                        @endfor
                    </select>
                    @error('nivel')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
